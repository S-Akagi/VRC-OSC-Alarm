...
// タイマー管理
pub async fn calculate_and_set_next_alarm(state: &AppStateMutex, timer_manager: &TimerManagerMutex) {
    // 既存タイマーをキャンセル
    {
...
// 公開タイマーイベント処理関数
pub async fn handle_timer_event(state: AppStateMutex, timer_manager: TimerManagerMutex, event: TimerEvent) {
    match event {
